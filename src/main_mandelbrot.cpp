...
//    float sizeX = 2.0f;
    ...
    context.activate();
    {
        ...
        std::vector<unsigned int> workGroupSize = { 2, 64 };
        std::vector<unsigned int> global_work_size = {
                (width + workGroupSize[0] - 1) / workGroupSize[0] * workGroupSize[0],
                (height + workGroupSize[1] - 1) / workGroupSize[1] * workGroupSize[1]
        };

        gpu::WorkSize ws = gpu::WorkSize(workGroupSize[0], workGroupSize[1],
