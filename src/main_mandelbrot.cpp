...
//    float sizeX = 2.0f;
    ...
    context.activate();
    {
        ...
        size_t workGroupSize = 16 * 16;
        unsigned int global_work_size = (n + workGroupSize - 1) / workGroupSize * workGroupSize;
        gpu::gpu_mem_32f canvas_gpu;
        canvas_gpu.resizeN(n);

        timer t;
        for (unsigned int i = 0; i < benchmarkingIters; ++i) {
            kernel.exec(gpu::WorkSize(workGroupSize, 1, width, height),
                        canvas_gpu, width, height,centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
                        sizeX, sizeY, iterationsLimit, 1);
            canvas_gpu.readN(gpu_results.ptr(), n);
            t.nextLap();
        }
        size_t flopsInLoop = 10;
        size_t maxApproximateFlops = width * height * iterationsLimit * flopsInLoop;
        ...
        std::cout << "    Real iterations fraction: " << 100.0 * realIterationsFraction / n << "%" << std::endl;
