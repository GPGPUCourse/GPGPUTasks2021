...
//    float sizeX = 2.0f;
    ...
    context.activate();
    {
        ...
        gpu::gpu_mem_32f gpuBuff; // выделяем пасять под вычисления
        gpuBuff.resizeN(width * height);

        timer t;
        for (int i = 0; i < benchmarkingIters; ++i) {
            kernel.exec(gpu::WorkSize(16, 16, width, height),
                        gpuBuff, width, height,
                        centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
                        sizeX, sizeY,
                        iterationsLimit, 0);
            t.nextLap();
        ...
        gpuBuff.readN(gpu_results.ptr(), width * height); // загружаем результат вычислений
