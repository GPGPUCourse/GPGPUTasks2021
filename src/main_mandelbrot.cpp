...
int main(int argc, char **argv)
{
    ...
    context.activate();
    {
        ...
        bool printLog = false; // Лог пустой
        ...
        {
            // Создаем буфер в видеопамяти для результата
            gpu::gpu_mem_32f results;
            results.resizeN(width * height);

            // Чтобы была coalesced запись выбираем размер рабочей группы из: (128,1) | (64,2) | (32,4)
            unsigned int workGroupSizeX = 128;
            unsigned int workGroupSizeY = 1;

            // Выполняем кернел в цикле для бенчмарка
            timer t;
            for (int i = 0; i < benchmarkingIters; ++i) {
                kernel.exec(
                    gpu::WorkSize(workGroupSizeX, workGroupSizeY, width, height),
                    results,
                    width, height,
                    centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
                    sizeX, sizeY,
            ...
            // Запишем результат в обычную память
