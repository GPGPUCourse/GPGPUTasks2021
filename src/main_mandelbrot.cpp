...
//    float sizeX = 2.0f;
...

    {
        ...
        std::cout << "CPU: " << 1.0f * maxApproximateFlops / gflops / t.lapAvg() << " GFlops" << std::endl;
    ...
    context.activate();
    {
        ...
        gpu::gpu_mem_32f out;
        out.resizeN(width * height);

        const unsigned workGroupSize = 128;
        const unsigned globalWorkSize = (width * height + workGroupSize - 1) / workGroupSize * workGroupSize;
        ...
        out.readN(gpu_results.ptr(), width * height);

        size_t flopsInLoop = 10;
        size_t maxApproximateFlops = width * height * iterationsLimit * flopsInLoop;
        size_t gflops = 1000*1000*1000;
        std::cout << "GPU: " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
        std::cout << "GPU: " << 1.0f * maxApproximateFlops / gflops / t.lapAvg() << " GFlops" << std::endl;
