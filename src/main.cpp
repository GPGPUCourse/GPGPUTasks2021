...
int main()
{
    ...
    cl_device_id device;
    bool gpuDeviceFound = false;

    for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        cl_platform_id _platform = platforms[platformIndex];

        cl_uint numDevices = 0;
        OCL_SAFE_CALL(clGetDeviceIDs(_platform, CL_DEVICE_TYPE_ALL, 0, nullptr, &numDevices));
        std::vector<cl_device_id> devices(numDevices, 0);
        OCL_SAFE_CALL(clGetDeviceIDs(_platform, CL_DEVICE_TYPE_ALL, numDevices, devices.data(), nullptr));
        if (platformIndex == 0) {
            device = devices[0];
        }
        ...
        for (int deviceIndex = 0; deviceIndex < numDevices; ++deviceIndex) {
            size_t typeSize;
            cl_int type;
            cl_device_id _device = devices[deviceIndex];
            OCL_SAFE_CALL(clGetDeviceInfo(_device, CL_DEVICE_TYPE, 0, nullptr, &typeSize));
            OCL_SAFE_CALL(clGetDeviceInfo(_device, CL_DEVICE_TYPE, typeSize, &type, nullptr));
            if (type == CL_DEVICE_TYPE_GPU && !gpuDeviceFound) {
                platform = _platform;
                device = _device;
                gpuDeviceFound = true;
            }
    ...
    std::cout << (gpuDeviceFound ? "GPU device found" : "no GPU device") << std::endl;
    ...
    cl_command_queue commandQueue = clCreateCommandQueue(context, device, 0, &error_code);
    OCL_SAFE_CALL(error_code);
    ...
    cl_mem aBuffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, as.data(), &error_code);
    OCL_SAFE_CALL(error_code);
    cl_mem bBuffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, bs.data(), &error_code);
    OCL_SAFE_CALL(error_code);
    cl_mem cBuffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, cs.data(), &error_code);
    OCL_SAFE_CALL(error_code);
    ...
    // TODO 7 Создайте OpenCL-подпрограмму с исходниками кернелаlong
    // см. Runtime APIs -> Program Objects -> clCreateProgramWithSource
    // у string есть метод c_str(), но обратите внимание, что передать вам нужно указатель на указатель

    const char* source = kernel_sources.c_str();
    cl_program program = clCreateProgramWithSource(context, 1, &source, nullptr, &error_code);
    OCL_SAFE_CALL(error_code);
    // TODO 8 Теперь скомпилируйте программу и напечатайте в консоль лог компиляции
    // см. clBuildProgram

    OCL_SAFE_CALL(clBuildProgram(program, 1, &device, nullptr, NULL, nullptr));
    ...
    
    {
        ...
        std::cout << "VRAM -> RAM bandwidth: " << (n * sizeof(float)) / ((1 << 30) * t.lapAvg()) << " GB/s" << std::endl;
    }
