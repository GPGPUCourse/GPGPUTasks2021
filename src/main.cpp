...
int main()
{
    ...
    cl_mem as_gpu, bs_gpu, cs_gpu;
    as_gpu = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, as.data(), &errcode_ret);
        OCL_SAFE_CALL(errcode_ret);
    bs_gpu = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, bs.data(), &errcode_ret);
        OCL_SAFE_CALL(errcode_ret);
    cs_gpu = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(float) * n, nullptr, &errcode_ret);
        OCL_SAFE_CALL(errcode_ret);


    // Выполните TODO 5 (реализуйте кернел в src/cl/aplusb.cl)
    ...
    {
        const char* arr[1];
        arr[0] = kernel_sources.c_str();
        program = clCreateProgramWithSource(context, 1, arr, nullptr, &errcode_ret);
        OCL_SAFE_CALL(errcode_ret);
    }

    // Теперь скомпилируйте программу и напечатайте в консоль лог компиляции
    // см. clBuildProgram
    errcode_ret = clBuildProgram(program, 1, &device, "", nullptr, nullptr);
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        for (unsigned int i = 0; i < 30; ++i) {
            cl_event event;
            OCL_SAFE_CALL(clEnqueueNDRangeKernel(command_queue, kernel, 1, nullptr,
        ...
        // - Среднее время выполнения кернела равно t.lapAvg() секунд
        std::cout << "VRAM bandwidth: " << 3 * n * sizeof(float) / t.lapAvg() / (1 << 30) << " GiB/s" << std::endl;
    }
    ...
    // Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            OCL_SAFE_CALL(clEnqueueReadBuffer(command_queue, cs_gpu, CL_TRUE, 0,
                                              sizeof(float) * n, cs.data(),
                                              0, nullptr, nullptr));
            t.nextLap();
        }
        std::cout << "Result data transfer time: " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
        std::cout << "VRAM -> RAM bandwidth: " << n * sizeof(float) / t.lapAvg() / (1 << 30) << " GiB/s" << std::endl;
    }
