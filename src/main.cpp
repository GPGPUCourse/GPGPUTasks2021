...
int main() {
    ...
    cl_mem as_buff = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, as.data(),
                                    &errcode);
    cl_mem bs_buff = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, bs.data(),
                                    &errcode);
    cl_mem cs_buff = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, cs.data(),
                                    &errcode);
    ...
    const char* pProgram = kernel_sources.c_str();
    const char** ppProgram = &pProgram;

    cl_program program = clCreateProgramWithSource(context, 1, ppProgram, nullptr, &errcode);
    ...
    OCL_SAFE_CALL(clGetProgramBuildInfo(program, device, CL_PROGRAM_BUILD_STATUS, sizeof(cl_build_status), &status,
                                        nullptr));
    ...
    size_t kernelArgs = 0;
    OCL_SAFE_CALL(clGetKernelInfo(kernel, CL_KERNEL_NUM_ARGS, sizeof(kernelArgs), &kernelArgs, nullptr));
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            ...
            OCL_SAFE_CALL(clEnqueueNDRangeKernel(commandQueue, kernel, 1, nullptr, &global_work_size, nullptr,
                                                      0, nullptr, &event));
    ...
    OCL_SAFE_CALL(clReleaseMemObject(as_buff));
    OCL_SAFE_CALL(clReleaseMemObject(bs_buff));

    std::vector<float> cs_cpu(n);
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            ...
            OCL_SAFE_CALL(clEnqueueReadBuffer(commandQueue, cs_buff, CL_TRUE, 0, sizeof(float) * n, cs_cpu.data(),
                                              0, nullptr, &event));
