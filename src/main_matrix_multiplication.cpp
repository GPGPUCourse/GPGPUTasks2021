...
int main(int argc, char **argv)
{
...

    {
        timer t;
        for (int iter = 0; iter < benchmarkingIters; ++iter) {
            unsigned int group_size = 16;
            unsigned int work_size_x = (M + group_size - 1) / group_size * group_size;
            unsigned int work_size_y = (K + group_size - 1) / group_size * group_size;

            auto workSize = gpu::WorkSize(group_size, group_size, work_size_x, work_size_y);
